    ...
    .then(data => {
         ...
         function buildDashboardPanelHtml(tabId, tabTitle) {
            return `
                <section class="upper-section redesigned-upper-section">
                    <div class="dashboard-chart-card age-chart-card">
                        ...
                        <div class="chart-card-text">
                            ...
                            <p class="chart-description">
                                This chart shows how the selected patient population is distributed across age groups.
                            </p>
                        </div>
                    </div>

                    <div class="statistic-column">
                        <div class="statistic-card depression-stat">
                            <div class="number" id="${tabId}-depression-count">Loading...</div>
                            <p>People with Depression</p>

                            <button class="view-patients-button" data-condition="depression" data-tab-id="${tabId}" type="button">
                                <span class="view-arrow">↑</span>
                                <span class="view-text">View patients</span>
                            </button>
                        </div>

                        <div class="statistic-card diabetes-stat">
                            <div class="number" id="${tabId}-diabetes-count">Loading...</div>
                            <p>People with Diabetes</p>

                            <button class="view-patients-button" data-condition="diabetes" data-tab-id="${tabId}" type="button">
                                <span class="view-arrow">↑</span>
                                <span class="view-text">View patients</span>
                            </button>
                        </div>

                        <div class="statistic-card htn-stat">
                            <div class="number" id="${tabId}-htn-count">Loading...</div>
                            <p>People with Hypertension</p>

                            <button class="view-patients-button" data-condition="htn" data-tab-id="${tabId}" type="button">
                                <span class="view-arrow">↑</span>
                                <span class="view-text">View patients</span>
                            </button>
                        </div>

                        <div class="statistic-card obesity-stat">
                            <div class="number" id="${tabId}-obesity-count">Loading...</div>
                            <p>People with Obesity</p>

                            <button class="view-patients-button" data-condition="obesity" data-tab-id="${tabId}" type="button">
                                <span class="view-arrow">↑</span>
                                <span class="view-text">View patients</span>
                            </button>
                        </div>
                    ...
                    <div class="dashboard-chart-card language-chart-card">
                        <div class="chart-card-visual">
                        ...
                        <div class="chart-card-text">
                            ...
                            <p class="chart-description">
                                This chart shows the preferred language breakdown for the selected patient population.
                            </p>
                        </div>
                    </div>
                </section>
                ...
                <div class="lower-section tab-lower-section">
                    <div class="task-container">
                        ...
                        <ul class="container-ul" id="${tabId}-task-list"></ul>

                        <button class="filter-button add-task tab-add-task-button" data-tab-id="${tabId}" type="button">
                            Add Task
                        </button>
                    </div>

                    <div class="chronic-condition-container">
                        <div class="list-header">
                            <h2 class="list-title">Chronic Condition Dashboard</h2>
                        </div>

                        <ul class="container-ul">
                            <div class="container-li">
                                <img src="images/clipboard.svg" alt="icon" class="task-icon">
                                <li class="task-label">Diabetes</li>
                                <div class="progress-container">
                                    <div class="progress-bar" id="${tabId}-diabetes-bar">
                                        <span id="${tabId}-diabetes-percent">Loading...</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="container-li">
                                <img src="images/clipboard.svg" alt="icon" class="task-icon">
                                <li class="task-label">Hypertension</li>
                                <div class="progress-container">
                                    <div class="progress-bar" id="${tabId}-htn-bar">
                                        <span id="${tabId}-htn-percent">Loading...</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="container-li">
                                <img src="images/clipboard.svg" alt="icon" class="task-icon">
                                <li class="task-label">Obesity</li>
                                <div class="progress-container">
                                    <div class="progress-bar" id="${tabId}-obesity-bar">
                                        <span id="${tabId}-obesity-percent">Loading...</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="container-li">
                                <img src="images/clipboard.svg" alt="icon" class="task-icon">
                                <li class="task-label">Depression</li>
                                <div class="progress-container">
                                    <div class="progress-bar" id="${tabId}-depression-bar">
                                        <span id="${tabId}-depression-percent">Loading...</span>
                                    </div>
                                </div>
                            </div>
                        </ul>
        ...
        function updateTabLowerDashboard(tabId) {
            ...
            if (filterState.pcp === "all") {
                ...
                const nameParts = filterState.pcp.split(" ");
                const lastName = nameParts.length > 1 ? nameParts[nameParts.length - 1] : filterState.pcp;

                if (centerOverviewTitleElement) {
                    centerOverviewTitleElement.textContent = `Dr. ${lastName}'s Overview`;
                }
            }
        ...
        function getTaskStorageKey(tabId) {
            const tabState = tabStates[tabId] || { filterState: getDefaultFilterState() };
            const filterState = tabState.filterState || getDefaultFilterState();
        ...
        function getCompletedStorageKey(tabId) {
            return getTaskStorageKey(tabId) + "__completed_count";
        }

        function getSavedTasks(tabId) {
            const storageKey = getTaskStorageKey(tabId);
        ...
        function saveTasksForTab(tabId) {
            const taskList = document.getElementById(`${tabId}-task-list`);

            if (!taskList) {
                return;
            }

            const tasks = Array.from(taskList.querySelectorAll(".task-label"))
                .map(label => label.textContent.trim())
                .filter(taskText => taskText !== "");

            localStorage.setItem(getTaskStorageKey(tabId), JSON.stringify(tasks));
        }

        function getCompletedCount(tabId) {
            return Number(localStorage.getItem(getCompletedStorageKey(tabId))) || 0;
        }

        function setCompletedCount(tabId, count) {
            localStorage.setItem(getCompletedStorageKey(tabId), String(count));

            const completedElement = document.getElementById(`${tabId}-completed-task-count`);

            if (completedElement) {
                completedElement.textContent = count.toLocaleString();
            }
        }

        function createTaskElement(tabId, taskText) {
            const taskItem = document.createElement("div");
            taskItem.className = "container-li task-item";
            ...
            check.addEventListener("click", () => {
                const completedCount = getCompletedCount(tabId) + 1;
                setCompletedCount(tabId, completedCount);
        ...
        function renderTasksForTab(tabId) {
            ...
            const savedTasks = getSavedTasks(tabId);
            taskList.innerHTML = "";

            savedTasks.forEach(taskText => {
                taskList.appendChild(createTaskElement(tabId, taskText));
            });

            setCompletedCount(tabId, getCompletedCount(tabId));
            saveTasksForTab(tabId);
        }
        ...
        function setupEvents() {
            ...
            document.addEventListener("click", event => {
                ...
                const addTaskButton = event.target.closest(".tab-add-task-button");

                if (addTaskButton) {
                    const tabId = addTaskButton.dataset.tabId;
                    const taskList = document.getElementById(`${tabId}-task-list`);
                    ...
                    const taskItem = createTaskElement(tabId, "New Task");
