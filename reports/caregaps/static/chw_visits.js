/* ==========================================================================
   CHW Visits Completed dashboard
   Fetches the monthly operational-activity snapshot JSON (produced by
   OperationalActivity.py) and renders the CHW metric section:
     - Stat row: total visits this month + distinct employee count
     ...
     - Daily-detail Tabulator table (one row per employee per day, as
       returned by CHW.sql / _run_and_rollup())
   ...
   payload.metrics.chw. See OperationalActivity.py's module docstring and
   fluoride.js (same snapshot, payload.metrics.fluoride).
   ========================================================================== */
...
let REPORT = null;
let METRIC = null; // REPORT.metrics.chw
...
async function loadData() {
  try {
    ...
    console.error('CHW Visits dashboard: failed to load', DATA_URL, err);
    renderLoadError(err);
    return;
  }

  METRIC = (REPORT.metrics && REPORT.metrics.chw) || null;
  if (!METRIC) {
    renderLoadError(new Error('operational_activity snapshot has no metrics.chw section'));
    return;
  }
...
function renderLoadError(err) {
  ...
  banner.innerHTML = `<span class="stat-label">Could not load ${DATA_URL}: ${err.message}. ` +
    `Check that the snapshot file exists and the path in chw_visits.js is correct.</span>`;
...
function buildEmployeeTable() {
  ...
  employeeTable = new Tabulator('#employeeTable', {
    ...
    columns: [
      ...
      { title: 'Visits this month', field: 'count', width: 180, sorter: 'number' },
    ],
  });
}
...
function wireDetailControls() {
  ...
  document.getElementById('downloadCsv').addEventListener('click', () => {
    if (!employeeTable) return;
    employeeTable.download('csv', 'chw_visits_by_employee.csv');
  });
}